...
current_revenue = float(input("请输入当前年度收入 (百万美元): "))
#年度收入决定了未来现金流的基数（简化估计）
...
# 计算自由现金流 (简化计算)
fcf_list = []
RATE = float(input("请输入假设的自由现金流与收入比值(百分比): ")) / 100
current_fcf = current_revenue * RATE  # 假设FCF利润率为输入值RATE
